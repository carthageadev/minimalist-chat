...
interface CRTTerminalProps {
    scale?: number; // Kept for compatibility but unused in fullscreen mode
}

const CRTTerminal: React.FC<CRTTerminalProps> = () => {
    ...
    useEffect(() => {
        if (bootSequence < 5) {
            const timeout = setTimeout(() => setBootSequence(prev => prev + 1), 800);
                ...
                <div className="screen-warp">
                    <div className="screen-inner">
                        <div className="screen-content flicker">
                            {currentScreen === 'boot' && (
                                <div className="amber-text h-full flex flex-col justify-center items-center">
                                    {bootSequence >= 1 && (
